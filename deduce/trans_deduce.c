...
typedef struct {
	long nexts[2];	// size_t next[2] but need to take into account the negativity of a litteral...
	size_t mark;
} Node;

typedef struct {
	size_t num_clauses;
	long * vars;		// size_t vars but need to take into account the negativity of a litteral...
	Node * nodes;
} Filter;
...
void domfil_add (Filter * fl, Cnf * src, size_t cl) {
	...
	fl->vars[abs(v0)-1] = (v0 < 0) ? 2 * cl : - 2 * cl;
	fl->vars[abs(v1)-1] = (v1 < 0) ? 2 * cl + 1 : - 2 * cl - 1;
}
...
void translation_deduce (Cnf * src, Translation * dst) {
	...
	dst->doms = realloc (dst->doms, sizeof (size_t) * (dst->num_doms + dst->num_cons + 1));
